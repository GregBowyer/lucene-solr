 ...
 * @lucene.experimental
 */
public final class IntrinsicsPostingsWriter extends PushPostingsWriterBase {
  ...
  public IntrinsicsPostingsWriter(SegmentWriteState state) throws IOException {
    final float acceptableOverheadRatio = PackedInts.COMPACT;

    String docFileName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
    ...
    try {
      ...
      //Intrinsics = new ForUtil(acceptableOverheadRatio, docOut);
      if (state.fieldInfos.hasProx()) {
        posDeltaBuffer = new int[MAX_DATA_SIZE];
        String posFileName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
            IntrinsicsPostingsFormat.POS_EXTENSION);
        ...
        if (state.fieldInfos.hasPayloads() || state.fieldInfos.hasOffsets()) {
          String payFileName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
              IntrinsicsPostingsFormat.PAY_EXTENSION);
  ...
  public void addPosition(int position, BytesRef payload, int startOffset, int endOffset) throws IOException {
    ...
    if (posBufferUpto == BLOCK_SIZE) {
      Intrinsics.writeBlock(posDeltaBuffer, posBufferUpto, encoded, posOut);

      if (writePayloads) {
        Intrinsics.writeBlock(payloadLengthBuffer, posBufferUpto, encoded, payOut);
        payOut.writeVInt(payloadByteUpto);
        payOut.writeBytes(payloadBytes, 0, payloadByteUpto);
        payloadByteUpto = 0;
      }
      if (writeOffsets) {
        Intrinsics.writeBlock(offsetStartDeltaBuffer, posBufferUpto, encoded, payOut);
        Intrinsics.writeBlock(offsetLengthBuffer, posBufferUpto, encoded, payOut);
      }
      posBufferUpto = 0;
    }
  }
