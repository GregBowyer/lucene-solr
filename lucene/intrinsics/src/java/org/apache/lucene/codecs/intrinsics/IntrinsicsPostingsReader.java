package org.apache.lucene.codecs.intrinsics;
...
import org.apache.lucene.codecs.intrinsics.IntrinsicsPostingsFormat.IntBlockTermState;
...
import org.apache.lucene.intrinsics.Intrinsics;
...
import static org.apache.lucene.codecs.intrinsics.IntrinsicsPostingsFormat.POS_CODEC;
import static org.apache.lucene.codecs.intrinsics.IntrinsicsPostingsFormat.TERMS_CODEC;
import static org.apache.lucene.codecs.intrinsics.IntrinsicsPostingsFormat.VERSION_CURRENT;
import static org.apache.lucene.codecs.intrinsics.IntrinsicsPostingsFormat.VERSION_START;
 ...
 * @lucene.experimental
 */
public final class IntrinsicsPostingsReader extends PostingsReaderBase {

  private static final long BASE_RAM_BYTES_USED =
      RamUsageEstimator.shallowSizeOfInstance(IntrinsicsPostingsReader.class);
  ...
  /** Sole constructor. */
  public IntrinsicsPostingsReader(SegmentReadState state) throws IOException {
    ...
    String docName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
        IntrinsicsPostingsFormat.DOC_EXTENSION);
    try {
      ...
      if (state.fieldInfos.hasProx()) {
        String proxName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, IntrinsicsPostingsFormat.POS_EXTENSION);
        ...
        if (state.fieldInfos.hasPayloads() || state.fieldInfos.hasOffsets()) {
          String payName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, IntrinsicsPostingsFormat.PAY_EXTENSION);
  ...
  final class BlockDocsEnum extends PostingsEnum {
    ...
    public BlockDocsEnum(FieldInfo fieldInfo) throws IOException {
      this.startDocIn = IntrinsicsPostingsReader.this.docIn;
      this.docIn = null;
    ...
    private void refillDocs() throws IOException {
      ...
      if (left >= BLOCK_SIZE) {
        Intrinsics.readBlock(docIn, encoded, docDeltaBuffer);

        if (indexHasFreq) {
          if (needsFreq) {
            Intrinsics.readBlock(docIn, encoded, freqBuffer);
          } else {
            Intrinsics.skipBlock(docIn); // skip over freqs
          }
        }
      } else if (docFreq == 1) {
    ...
    public int advance(int target) throws IOException {
      ...
      if (docFreq > BLOCK_SIZE && target > nextSkipDoc) {
        ...
        // always plus one to fix the result, since skip position in IntrinsicsSkipReader
  ...
  final class BlockPostingsEnum extends PostingsEnum {
    ...
    public BlockPostingsEnum(FieldInfo fieldInfo) throws IOException {
      this.startDocIn = IntrinsicsPostingsReader.this.docIn;
      this.docIn = null;
      this.posIn = IntrinsicsPostingsReader.this.posIn.clone();
      encoded = new byte[MAX_ENCODED_SIZE];
      indexHasOffsets = fieldInfo.getIndexOptions().compareTo(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >= 0;
      indexHasPayloads = fieldInfo.hasPayloads();
    }
    ...
    private void refillPositions() throws IOException {
      if (posIn.getFilePointer() == lastPosBlockFP) {
        ...
        Intrinsics.readBlock(posIn, encoded, posDeltaBuffer);
      }
    }
  ...
  final class EverythingEnum extends PostingsEnum {
    ...
    public EverythingEnum(FieldInfo fieldInfo) throws IOException {
      ...
      this.docIn = null;
      this.posIn = IntrinsicsPostingsReader.this.posIn.clone();
      this.payIn = IntrinsicsPostingsReader.this.payIn.clone();
      encoded = new byte[MAX_ENCODED_SIZE];
      indexHasOffsets = fieldInfo.getIndexOptions().compareTo(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >= 0;
      if (indexHasOffsets) {
    ...
    private void refillPositions() throws IOException {
      if (posIn.getFilePointer() == lastPosBlockFP) {
        ...
        if (indexHasPayloads) {
          if (needsPayloads) {
            Intrinsics.readBlock(payIn, encoded, payloadLengthBuffer);
            ...
            Intrinsics.skipBlock(payIn); // skip over lengths
        ...
        if (indexHasOffsets) {
          if (needsOffsets) {
            Intrinsics.readBlock(payIn, encoded, offsetStartDeltaBuffer);
            Intrinsics.readBlock(payIn, encoded, offsetLengthBuffer);
          } else {
            // this works, because when writing a vint block we always force the first length to be written
            Intrinsics.skipBlock(payIn); // skip over starts
            Intrinsics.skipBlock(payIn); // skip over lengths
          }
        }
      }
